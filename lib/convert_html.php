...
class TableCell extends Element
{
	...
	function TableCell($text, $is_template = FALSE)
	{
		...
		if ($text != '' && $text{0} == '#') {
			// ･ｻ･・簣ﾆ､ｬ'#'､ﾇｻﾏ､ﾞ､・ﾈ､ｭ､ﾏDiv･ｯ･鬣ｹ､ﾌ､ｷ､ﾆ､ﾟ､・			$obj = & Factory_Div($this, $text);
	...
	function toString()
	{
		...
		for ($ncol = 0; $ncol < $this->col; $ncol++) {
			...
			foreach (array_reverse(array_keys($this->elements)) as $nrow) {
				...
				while (--$rowspan) // ｹﾔｼ・ﾌ､ﾑｾｵ､ｹ､・					$this->types[$nrow + $rowspan] = $this->types[$nrow];
				$rowspan = 1;
			}
		}

		// colspan,style､ﾟﾄ・		$stylerow = NULL;
		foreach (array_keys($this->elements) as $nrow) {
			...
			foreach (array_keys($row) as $ncol) {
				...
				if ($stylerow !== NULL) {
					...
					while (--$colspan) // ﾎｹ･ｿ･､･・ﾑｾｵ､ｹ､・						$row[$ncol - $colspan]->setStyle($stylerow[$ncol]->style);
				}
				$colspan = 1;
			}
		}
